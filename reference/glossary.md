...
| 术语 | 说明 |
|------|------|
| **待确认 / 已确认** | 澄清文档中每条澄清项的**状态**。仅可填写此二值：`待确认` 表示待用户填写结论；`已确认` 表示用户已填写并可回写至其它文档。见 [templates/00-clarifications.md](../templates/00-clarifications.md)、[extract-clarification](../skills/extract-clarification/SKILL.md)。 |
| **C-xxx** | 澄清项 ID，如 C-001、C-002。由 agent 在向澄清文档新增行时分配，同需求目录内取已有最大编号 +1；若用户手改编号，以不重复 ID 为原则分配。见 [extract-clarification](../skills/extract-clarification/SKILL.md)。 |
...
| **澄清回写** | 将澄清文档中状态为「已确认」且已填结论的项，按归属文档与关联章节写回 01/02/03/04；回写前需 doc-agent-stance 表态，有冲突则 doc-agent-discussion 收敛，回写后须 whole-document review 并再做澄清提取，必要时触发级联。见 [writeback-clarification](../skills/writeback-clarification/SKILL.md)。 |
| **提取待澄清** | 从 01/02/03/04 中识别需用户确认或二义性点，在澄清文档中新增行（C-xxx），供用户填写。见 [extract-clarification](../skills/extract-clarification/SKILL.md)。 |
| **目标需求解析顺序** | 写文档或读需求目录时确定**目标需求目录**的优先级：用户显式指定 > 当前编辑文件所在目录 > `spec/.state.json` active 字段 > 仅有一需求目录 > 否则向用户确认并列出需求。见 [multi-requirement.md](multi-requirement.md)。 |
...
| **spec/.state.json** | 多需求管理的单一状态源，位于 `spec/.state.json`。其 `active` 字段保存当前默认需求目录路径；`requirements[]` 数组记录各需求状态、待澄清数、共享模块等元信息。未显式指定需求时，生成、级联、澄清、list 等以 `active` 路径为默认目标；若指向的目录不存在则视为未设置。`spec/.active`（旧版单行文件）仅作兼容只读回退，不再写入。见 [multi-requirement.md](multi-requirement.md)。 |

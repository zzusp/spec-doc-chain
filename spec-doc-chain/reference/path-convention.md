...
与 spec-agent 项目约定对齐（路径、文件名、全局记忆、澄清真源/镜像一致）：规范文档链生成的文档按**日期 + 需求名称**存放，推荐使用 **spec/** 根路径与**编号文件名**，便于与 spec-agent 工作区共用或迁移。

## 推荐格式（与 spec-agent 一致）
